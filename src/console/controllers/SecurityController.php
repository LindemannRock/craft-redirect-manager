...
class SecurityController extends Controller
{
    ...
    public function actionGenerateSalt(): int
    {
        ...
        if (!file_exists($envPath)) {
            ...
            $this->stdout("REDIRECT_MANAGER_IP_SALT=\"{$salt}\"\n\n", Console::FG_GREEN);
            return ExitCode::OK;
        }

        // Read current .env file
        $envContent = file_get_contents($envPath);
        $saltExists = preg_match('/^REDIRECT_MANAGER_IP_SALT=/m', $envContent);

        if ($saltExists) {
            $this->stdout("Existing REDIRECT_MANAGER_IP_SALT found in .env\n\n", Console::FG_YELLOW);
            ...
            $envContent = preg_replace(
                '/^REDIRECT_MANAGER_IP_SALT=.*$/m',
                'REDIRECT_MANAGER_IP_SALT="' . $salt . '"',
                $envContent
            );
            $action = "Updated";
        } else {
            ...
            $envContent .= 'REDIRECT_MANAGER_IP_SALT="' . $salt . '"' . "\n";
            $action = "Added";
        }
        ...
        $this->stdout("\n✓ {$action} REDIRECT_MANAGER_IP_SALT in .env file\n", Console::FG_GREEN);
