...
- **[JSON API](../developers/api-endpoints.md) @since(5.33.0)** — Expose enabled redirects as plain read-only JSON for static builds, SPAs, edge workers, or backend integrations that do not use GraphQL. [Testing tools](../resources/testing-tools.md) includes the live CP tester and downloadable Postman collection.
