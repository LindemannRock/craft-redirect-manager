...
| `undoWindowMinutes` | `int` | `60` | Time window in minutes for detecting immediate undo (`0`, `30`, `60`, `120`, `240`). `0` = disabled |
...
| `ipHashSalt` | `string\|null` | `null` | IP hash salt. Falls back to `REDIRECT_MANAGER_IP_SALT` env var |
...
| `refreshIntervalSecs` | `int\|null` | `null` | Dashboard auto-refresh interval in seconds (`null` = disabled) |
...
| `backupPath` | `string` | `'@storage/redirect-manager/backups'` | Local filesystem path for backups. Supports `$VARIABLE` env vars |
| `backupVolumeUid` | `string\|null` | `null` | Optional asset volume UID for storing backups |
