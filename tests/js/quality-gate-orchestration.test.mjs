...
function probeFixture() {
    const root = mkdtempSync(path.join(os.tmpdir(), 'redirect-manager-gate-'));
    const probePath = path.join(root, 'probe.sh');
    const logPath = path.join(root, 'constituents.log');
    writeFileSync(probePath, `#!/bin/sh\nprintf '%s:%s\\n' "$1" "$2" >> "$REDIRECT_MANAGER_GATE_PROBE_LOG"\nif [ "$1" = "$REDIRECT_MANAGER_GATE_FAIL_ID" ]; then exit 71; fi\nexit 0\n`, {mode: 0o700});
    chmodSync(probePath, 0o700);
    return {
        run(failId = '') { return spawnSync('node', [gatePath, '--probe', probePath], {cwd: pluginRoot, encoding: 'utf8', env: {...process.env, REDIRECT_MANAGER_GATE_PROBE_LOG: logPath, REDIRECT_MANAGER_GATE_FAIL_ID: failId}}); },
        ids() { try { return readFileSync(logPath, 'utf8').trim().split('\n').filter(Boolean).map((line) => line.split(':')[0]); } catch { return []; } },
...
function actFixture(content = workflow([checkout, trust, gate])) {
    const root = mkdtempSync(path.join(os.tmpdir(), 'redirect-manager-act-'));
    ...
    writeFileSync(path.join(binRoot, 'act'), `#!/bin/sh\nprintf '%s\\n' "$*" > "$REDIRECT_MANAGER_ACT_LOG"\ntouch "$REDIRECT_MANAGER_ACT_RESOURCES/container" "$REDIRECT_MANAGER_ACT_RESOURCES/network"\ncase " $* " in *" --rm "*) rm -f "$REDIRECT_MANAGER_ACT_RESOURCES"/* ;; esac\nexit 73\n`, {mode: 0o700});
    chmodSync(path.join(binRoot, 'act'), 0o700);
    return {
        run: () => spawnSync('/bin/bash', ['scripts/act-quality-gates'], {cwd: root, encoding: 'utf8', env: {...process.env, PATH: `${binRoot}:/usr/bin:/bin`, REDIRECT_MANAGER_ACT_LOG: logPath, REDIRECT_MANAGER_ACT_RESOURCES: resources}}),
...
test('aggregate declares each approved constituent exactly once', () => {
    ...
    assert.doesNotMatch(JSON.stringify(declared), /postgres|browser|provider/i);
});
...
test('DDEV container routing executes constituents directly without nesting DDEV', () => {
    const root = mkdtempSync(path.join(os.tmpdir(), 'redirect-manager-ddev-routing-'));
    const binRoot = path.join(root, 'bin');
    const logPath = path.join(root, 'commands.log');
    mkdirSync(binRoot);
    try {
        for (const executable of ['composer', 'bash', 'php', 'node', 'ddev']) {
            const status = executable === 'ddev' ? 79 : 0;
            const executablePath = path.join(binRoot, executable);
            writeFileSync(executablePath, `#!/bin/sh\nprintf '%s|%s|%s|%s\\n' "${executable}" "$*" "$REDIRECT_MANAGER_FIXTURE_SOURCE_VENDOR_ROOT" "$PWD" >> "$REDIRECT_MANAGER_DDEV_ROUTING_LOG"\nexit ${status}\n`, {mode: 0o700});
        ...
        const result = spawnSync(process.execPath, [gatePath], {
            ...
            env: {
                ...
                REDIRECT_MANAGER_DDEV_ROUTING_LOG: logPath,
            },
        });
        assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
        const commands = readFileSync(logPath, 'utf8');
        assert.doesNotMatch(commands, /^ddev\|/m);
        assert.match(commands, new RegExp(`^composer\\|check-platform-reqs --no-interaction\\|\\|${path.dirname(path.dirname(pluginRoot))}$`, 'm'));
        assert.match(commands, /^php\|tests\/Fixtures\/Project\/run\.php --no-progress\|.*\/vendor\|/m);
    } finally {
        rmSync(root, {recursive: true, force: true});
    }
});

test('aggregate preserves canonical order and propagates every constituent failure', async (context) => {
...
test('Act rejects missing or wildcard trust before launch', async (context) => {
