...
const analyticsMigration = 'src/migrations/m260825_000000_create_analytics_daily.php';
...
function packageFixture() {
    ...
    for (const relativePath of ['.gitignore', '.gitattributes', analyticsMigration]) {
...
test('customer archive preserves the approved runtime boundary', () => {
    ...
    assert.equal(files.includes(analyticsMigration), true);
...
test('isolated package fixtures use current candidate ignore and attribute bytes', () => {
    const current = packageFixture();
    try {
        for (const relativePath of ['.gitignore', '.gitattributes']) {
            assert.deepEqual(
                readFileSync(path.join(current.repository, relativePath)),
...
test('generated package-root composer lock remains outside the customer archive', () => {
    const current = packageFixture();
    let temporaryPath = '';
    try {
        writeFileSync(path.join(current.repository, 'composer.lock'), '{"_readme": ["generated by Composer"]}\n');
        const ignored = spawnSync('git', ['check-ignore', '--quiet', '--', 'composer.lock'], {cwd: current.repository});
        assert.equal(ignored.status, 0);
        const attribute = spawnSync('git', ['check-attr', 'export-ignore', '--', 'composer.lock'], {
            cwd: current.repository,
            encoding: 'utf8',
        });
        assert.equal(attribute.status, 0, attribute.stderr);
        assert.equal(attribute.stdout.trim(), 'composer.lock: export-ignore: set');
        const before = gitState(current.repository);
        const files = checkPackageExport(current.repository, {onTemporaryPath: (value) => { temporaryPath = value; }});
        assert.equal(files.length, approvedCustomerArchiveFileCount);
...
test('required analytics migration cannot be deleted or export-ignored', async (context) => {
    await context.test('deleted migration', () => {
        const current = packageFixture();
        try {
            rmSync(path.join(current.repository, analyticsMigration));
            assert.throws(
                () => checkPackageExport(current.repository),
                /missing runtime file: src\/migrations\/m260825_000000_create_analytics_daily\.php/,
            );
        } finally {
            rmSync(current.root, {recursive: true, force: true});
        }
