...
use lindemannrock\redirectmanager\tests\Support\DisposableCraftProject;
...
{
    $environmentName = 'REDIRECT_MANAGER_FIXTURE_SOURCE_VENDOR_ROOT';
...
function lifecycleProbe(string $packageRoot, string $runnerPath, string $vendorRoot): array
{
    ...
    if (($failure['failure'] ?? null) !== 'Synthetic disposable runner failure.'
        || ($failure['cleanup'] ?? null) !== ['projectRemoved' => true, 'databaseRemoved' => true, 'grantRemoved' => true]) {
    ...
    $readyPath = rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . '/redirect-manager-interrupt-' . bin2hex(random_bytes(8)) . '.json';
    $environment = [];
    ...
    try {
        ...
        $expectedProjectRoot = '#^' . preg_quote(rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR), '#')
            . '/redirect-manager-fixture-[a-f0-9]{16}$#';
        if (!is_string($databaseName) || !preg_match('/^rm_post2d_[a-f0-9]{16}$/', $databaseName)
        ...
        $pdo = new PDO('mysql:host=db;port=3306;charset=utf8mb4', 'root', 'root', [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
