...
use lindemannrock\redirectmanager\web\assets\analytics\AnalyticsAsset;

/**
 * Covers static asset delivery for Redirect Manager analytics.
 *
 * @since 5.41.0
 */
final class AssetDeliveryTest extends TestCase
{
    public function testInstalledAliasResolvesToPackageAssets(): void
    {
        ...
        $aliasRoot = realpath(Craft::getAlias('@lindemannrock/redirectmanager'));
        ...
        self::assertFileExists($aliasRoot . '/web/assets/analytics/dist/analytics.js');
    }

    public function testAnalyticsBundleResolvesAliasWithoutPluginDatabaseOrPublicationState(): void
    {
        $originalAlias = Craft::getAlias('@lindemannrock/redirectmanager');
        $originalAssetManager = Craft::$app->getAssetManager();
        $originalDb = Craft::$app->getDb();
        $originalPlugin = RedirectManager::$plugin;
        $aliasRoot = $this->createTrackedTempDirectory('redirect-manager-asset-package-');
        $offlineDb = new Connection([
            'dsn' => 'unsupported:redirect-manager-asset-test',
        ]);
        ...
        try {
            Craft::setAlias('@lindemannrock/redirectmanager', $aliasRoot);
            Craft::$app->set('assetManager', $offlineAssetManager);
            Craft::$app->set('db', $offlineDb);
            RedirectManager::$plugin = null;

            $bundle = new AnalyticsAsset();

            self::assertNull(RedirectManager::$plugin);
            ...
            RedirectManager::$plugin = $originalPlugin;
            Craft::$app->set('db', $originalDb);
            Craft::$app->set('assetManager', $originalAssetManager);
            Craft::setAlias('@lindemannrock/redirectmanager', $originalAlias);
        }
    }

    public function testPrepublishedAnalyticsUrlsRenderOnceWithBaseDependencyFirst(): void
    {
        $originalAssetManager = Craft::$app->getAssetManager();
        $view = Craft::$app->getView();
        $sourceRoot = Craft::getAlias('@lindemannrock/redirectmanager');
        $baseSourceRoot = Craft::getAlias('@lindemannrock/base');
        $baseAnalyticsUrl = 'https://cdn.example.test/base/analytics';
        $redirectAnalyticsUrl = 'https://cdn.example.test/redirect-manager/analytics';
        $assetManager = new class([ 'basePath' => $sourceRoot, 'baseUrl' => '/unavailable-runtime-assets', 'appendTimestamp' => false, 'bundles' => [ BaseAnalyticsAsset::class => [ 'basePath' => $baseSourceRoot . '/web/assets/analytics/dist', 'baseUrl' => $baseAnalyticsUrl, ], AnalyticsAsset::class => [ 'basePath' => $sourceRoot . '/web/assets/analytics/dist', 'baseUrl' => $redirectAnalyticsUrl, ], ], ]) extends AssetManager {
            /** @var list<string> */
            public array $publicationPaths = [];
        ...
        try {
            ...
            self::assertSame($redirectAnalyticsUrl, $view->assetBundles[AnalyticsAsset::class]->baseUrl);
            ...
            self::assertSame([
                ...
                $redirectAnalyticsUrl . '/analytics.js',
            ], $scriptUrls, $bodyHtml);
            self::assertSame([], $assetManager->publicationPaths);
        } finally {
            $view->clear();
    ...
    public function testCustomerArchivesIncludeEveryBundleAsset(): void
    {
        ...
        $packageRoot = dirname(__DIR__, 2);
        $archiveRoot = $this->createTrackedTempDirectory('redirect-manager-asset-archive-');
